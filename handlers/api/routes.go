...
func RegisterAndStart(logger *zap.Logger, db *gorm.DB, sess *discordgo.Session) error {
	...
	e.Use(middleware.CORSWithConfig(middleware.CORSConfig{
		...
		AllowHeaders: []string{echo.HeaderOrigin, echo.HeaderContentType, echo.HeaderAccept, echo.HeaderAuthorization, headerGuildID},
	}))
	...
	oauthCfg := discordOAuth2ConfigFromEnv()
	oauthRoutesEnabled := oauthEnvComplete() && jwtKey != ""
	if oauthEnvComplete() && jwtKey == "" {
		logger.Warn("Discord OAuth env vars are set but JWT_SIGNING_KEY is empty; OAuth routes and Bearer auth are disabled.")
	}

	if oauthRoutesEnabled {
		oauthDeps := newOAuthDeps(logger, userSessionRepo, oauthCfg, jwtKey)
		e.GET("/auth/discord", oauthDeps.handleAuthDiscordStart)
		e.GET("/auth/discord/callback", oauthDeps.handleAuthDiscordCallback)
		e.POST("/auth/refresh", oauthDeps.handleAuthRefresh)
	}

	e.GET("/metrics", groprometheus.PrometheusHandler())

	authMw := AuthMiddleware(AuthMiddlewareDeps{
		ApiKeyRepo:   apiClientRepo,
		Logger:       logger,
		OAuthEnabled: oauthRoutesEnabled,
		JWTKey:       jwtKey,
		BotSession:   sess,
	})
	...
	if oauthRoutesEnabled {
		...
		protected.GET("/guilds", gh.handleGuilds)
	}

	protected.GET("/grocery-lists", func(c echo.Context) error {
	...
	protected.DELETE("/groceries/:id", func(c echo.Context) error {
		defer handlers.Recover(logger)
		authContext := c.(*AuthContext)
	...
	protected.POST("/groceries", func(c echo.Context) error {
		authContext := c.(*AuthContext)
		ctx := c.Request().Context()
		guildID := authContext.GuildID
	...
	protected.GET("/registrations", func(c echo.Context) error {
		defer handlers.Recover(logger)
		authContext := c.(*AuthContext)
		guildID := authContext.GuildID
